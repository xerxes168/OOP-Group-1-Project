...
public class Terrain extends Entity implements iMovable, iCollidable{
	...
	private static final float CELL_WIDTH = Gdx.graphics.getWidth() / (float) GRID_COLS;
	private static final float CELL_HEIGHT = Gdx.graphics.getHeight() / (float) GRID_ROWS;
    private static final float TERRAIN_SIZE = Gdx.graphics.getWidth() / 12f;
	private float currentxPos;
	private float currentyPos;
    private static final float SPEED = 33;
    int numberOfLily = 5; // Number of lily to create
    ...
    //Track which grid cells have been used (if you want unique cells)
    public static ArrayList<Terrain> spawnTerrains(int numberOfLily, float scrollSpeed) {
        ...
        for (int j = 0; j < numberOfLily; j++) {
            ...
            // If we exceed attempts, we break out of the loop.
            if (attempts >= 100) {
                break;
            }
